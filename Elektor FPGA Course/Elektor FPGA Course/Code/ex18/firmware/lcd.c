...
//  LCD_wait_ready ();
    LCD_ctl_reg = 0x00;     // initialize control-lines
    LCD_wait_ready();
    LCD_write_cmd (0x38);   // 8-bit 1/16 Dutycycle 5x7 dots
    LCD_wait_ready();
    LCD_write_cmd (0x38);   // 8-bit 1/16 Dutycycle 5x7 dots
    LCD_wait_ready();
//  LCD_write_cmd (0x01);
//  LCD_wait_ready();
    ...
    LCD_wait_ready();
    LCD_write_cmd (0x06);   // Cursor shifts right
    LCD_wait_ready();
    LCD_write_cmd (0x0c);   // Display ON, cursor ON, blink OFF
    LCD_wait_ready();
    LCD_write_cmd (0x40);   // CGRAM pointer 0
    LCD_wait_ready();
    LCD_write_cmd (0x80);   // DDRAM pointer 0
    LCD_wait_ready();
    LCD_write_cmd (0x01);
    LCD_wait_ready();
    LCD_ctl_reg = 0x00;     // initialize control-lines
